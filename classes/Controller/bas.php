...
/* 25.02.2025 ДОбавлены колонки с указанием количества карт для записи и для удаления.
* для этого в контроллере добавлыен методы:
* - getCardListCount - формирует массив с количество карт для записи и удалдения 
* - counterHelper - вспомогательный метод для получения значения из базы данных.
*/

class Controller_bas extends Controller_Template {
	...
	public function before()
	{
			...
			try {
				...
				Log::instance()->add(Log::DEBUG, 'error db connect: ' . ($e->getMessage()));	
				$message = "Проблема с подключением к базе данных, подробности смотреть в log файле";
				$this->redirect('errorpage?err=' . urlencode($message));
			}
			
			$this->template->title = 'BAS-IP';
	}
	...
	{
		//echo Debug::vars('36'); exit;
		$id_dev = $this->request->query('id_dev') === null ? null : $this->request->query('id_dev');
		$token = Profiler::start('test', 'profiler');
		$deviceList=Model::factory('bas')->getBasDeviceList();//таблица состояния панелей
		
		//echo Debug::vars('37',Arr::get($deviceList, 'status'),  $deviceList); exit;
		if(Arr::get($deviceList, 'status')== 0){
		
		$content = View::factory('bas/index', array(
		...
		$this->template->content = $content;
			...
			$message = "Ошибка при получение списка устройств.";
			Log::instance()->add(Log::DEBUG,  iconv('UTF-8','windows-1251', $message));	
	...
	public function action_getListCard_()// получить список карт для указанного id_dev и записать/удалить их из панели
	{
			...
			case 'bas_changeLogin':
				$data=Validation::factory($_POST);
					$data->rule('login', 'not_empty')
						...
						->rule('password', 'regex', array(':value', '/^[[:graph:]]+$/'))
					...
					{
					//echo Debug::vars('93 OK', $data); exit;	
					//echo Debug::vars('93 OK', $data->errors('validation')); exit;
							$model_bas = Model::factory('Bas'); // Создание экземпляра модели Bas
							$login = $data['login'];
							$password = $data['password'];
							$id_dev = $data['id_dev'];
							$result = $model_bas->updateData($login, $password, $id_dev); // Вызов метода updateData() из модели
						if($result == 0){
							...
							Session::instance()->set('alertErr', $data = __('sql_err_388', array('id_dev'=>Arr::get($data,'id_dev'))));
							$this->redirect('/');
						}
					} else {

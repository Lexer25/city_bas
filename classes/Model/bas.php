...
/*
12.03.2025
Добавлено извление значение ONLINE из таблицы bas_param.
17.04.2024
Модель предназначена для работы с базой данных.
*/
...
class Model_Bas extends Model
{
	...
	public function getBasDeviceList($serverList = null)// получить id_dev контроллеров (вызывных панелей) типа bas
	{
		if(is_null($serverList))
		{
			$sql='select d.id_dev, d.name, 
				...
				bp3.INSERTTIME as INSERTTIME, 
				bp4.strvalue as LOGIN, 
				bp5.strvalue as PASS, 
				bp6.intvalue as ONLINE, 
				
				COALESCE(count(case when cd.operation=1 then 1 end), 0) AS count1,
                COALESCE(count(case when cd.operation=2 then 1 end), 0) AS count2
				...
				left join bas_param bp3 on bp3.id_dev=d.id_dev  and bp3.param=\'ABOUT\'
				 ...
				 left join bas_param bp6 on bp6.id_dev=d.id_dev  and bp6.param=\'ONLINE\'
	...
	//echo Debug::vars('134', $id_dev);exit;
		...
		if (!empty($id_dev)) {
			...
			$sql_insert_password = 'INSERT INTO bas_param (ID_DEV, PARAM, STRVALUE) VALUES (:id_dev, \'PASS\',  \''. $password . '\')';
